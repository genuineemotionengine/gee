 ...
 * Single self-contained file. Drop in as public/js/spaces.js
 * Requires: featureModal, featureModalBackdrop, featureModalTitle,
 *           featureModalBody, featureModalClose in the DOM.
 */
...
const GeeSpaces = (() => {
    ...
    // ── Player context line (#stream / #renderer in the main player UI) ──────
    ...
    function openModal(mode) {
        ...
        modal.className = modal.className
            .replace(/\bsearch-modal-active\b/g, '')
            .replace(/\bspaces-modal-active\b/g, '')
            .trim();
    ...
    function renderCurrent() {
        ...
        if (cur.space_type === 'room') {
            name = roomName(cur.space_id);
        } else if (cur.space_type === 'renderer') {
            name = rendererName(cur.space_id);
        }
        ...
        return `
            <section class="spaces-current-card">
                ...
                <div class="spaces-current-main">
                    <div>
                        <div class="spaces-current-name">${esc(name)}</div>
    ...
    // ── Render: standalone renderers ─────────────────────────────────────────
    function renderStandalone() {
        const renderers = state.data?.standalone_renderers || [];
        const cur       = state.data?.current || {};
    ...
    // ── Render: registered renderers (renderers mode) ────────────────────────
    ...
    async function open(mode = 'multiroom') {
        ...
        try {
            ...
            const body = $('featureModalBody');
            if (body) {
                body.innerHTML = `<div class="spaces-empty spaces-error">${esc(err.message || 'Failed to load listening spaces')}</div>`;
            }
        }
    }

    // ── Click handler (single delegated listener) ────────────────────────────
    async function handleClick(e) {
        ...
        // Only act on buttons/selects inside the modal body
        const body = $('featureModalBody');
        ...
        switch (action) {
            ...
            case 'delete-room': {
                ...
                if (!confirm(`Delete "${name}"?\n\nThis will remove the room. Renderers will become standalone.`)) return;
                await run('delete-room', {
                    room_id: btn.dataset.roomId,
                }, 'Room deleted');
                break;
            }
    ...
    function init() {
        ...
        // Keep player context line fresh even when modal is closed
